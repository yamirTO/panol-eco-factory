...
// ============================================================
//  EDITAR (solo admin) - CON IMÁGENES MÚLTIPLES
// ============================================================
...
function saveEdit() {
    ...
    if (imagenesTemporales.length > 0) {
        ...
        updatedItem.imagen = imagenesTemporales[0]; // Compatibilidad
    } else {
    ...
    if (imagenes.length === 0) {
        grid.innerHTML = `
            <div class="imagenes-empty">
                <span class="empty-img-icon">📷</span>
                <span style="font-size:12px;font-weight:600;">Sin imágenes</span>
                <span style="font-size:10px;color:var(--sub);">Agregá fotos del componente</span>
            </div>
        `;
        return;
    }
...
function cargarMultiplesImagenes(event) {
    ...
    if (imagenesTemporales.length + files.length > 5) {
        showToast('⚠️ Máximo 5 imágenes por componente', false);
        return;
    }
    ...
    files.forEach(file => {
        ...
        reader.onerror = function() {
            ...
            if (cargadas + errores === files.length) {
                renderImagenesGrid(imagenesTemporales);
                showToast('⚠️ Error al cargar algunas imágenes', false);
            }
        };
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        ...
        input.onchange = function(e) {
            if (e.target.files && e.target.files[0]) {
                ...
                cargarMultiplesImagenes(fakeEvent);
            }
            input.remove();
        };
        input.click();
    } else {
        document.getElementById('editMultipleImages').click();
    }
}
...
function abrirVisorImagen(index) {
    ...
    overlay.addEventListener('click', function(e) {
        if (e.target === overlay) cerrarVisorImagen();
    });
    ...
    if (isCreatingNewItem) {
        ...
        if (currentUser && USUARIOS_LOCALES[currentUser.username]) {
            items.push({
                ...
                minimo, maximo, ubicacion, planta, critico, obs: '', imagenes: []
            });
            localStorage.setItem('panol_items', JSON.stringify(items));
            movs.unshift({
                id: Date.now(),
...
function openBackupModal() {
    if (currentUser?.rol !== 'admin') { showToast('Solo administradores', false); return; }
...
function restoreBackup(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            let movsData = [];
            if (wsMovs) movsData = XLSX.utils.sheet_to_json(wsMovs);
...
function confirmRestore() {
    ...
    apiCall('/api/backup/restore', {
            method: 'POST',
            body: JSON.stringify({ items: pendingRestoreData.items, movimientos: pendingRestoreData.movs, usuarios: {} })
        })
        .then(() => {
            showLoading(false);
            document.getElementById('restoreInfo').style.display = 'none';
...
function openImportModal() {
    if (currentUser?.rol !== 'admin') { showToast('Solo administradores pueden importar', false); return; }
...
function mapearColumnas(headers) {
    ...
    headers.forEach((h, i) => {
        ...
        if (n.includes('stockinicial') || n === 'stockinicial' || n === 'stock_inicial' || n === 'stock inicial') map.stock = i;
    ...
    reader.onload = function(e) {
        try {
            ...
            for (let i = 0; i < Math.min(5, rows.length); i++) {
                if (rows[i].some(c => typeof c === 'string' && c.trim())) { hdrIdx = i; break; }
            }
            const map = mapearColumnas(rows[hdrIdx]);
            if (map.codigo === undefined || map.descripcion === undefined) { showImportError('Columnas obligatorias: Código y Descripción'); return; }
            ...
            previewContainer.innerHTML = `
                <div style="font-size:12px;font-weight:700;color:var(--sub);margin-bottom:8px;">Vista previa (${Math.min(5, importData.length)} de ${importData.length})</div>
                <table class="preview-table">
                    <thead><tr><th>Código</th><th>Descripción</th><th>Stock</th><th>Mín</th><th>Máx</th><th>Crítico</th></tr></thead>
                    <tbody>${importData.slice(0,5).map((r,i) => `
...
function confirmImport() {
    ...
    const promises = importData.map(item => {
        ...
        if (existe) {
            return apiCall(`/api/items/${item.codigo}`, { method: 'PUT', body: JSON.stringify(item) }).catch(() => null);
        } else {
            return apiCall('/api/items', { method: 'POST', body: JSON.stringify(item) }).catch(() => null);
        }
    });
    Promise.allSettled(promises).then(results => {
        ...
        showToast(`✅ ${procesados} ítems procesados`);
        loadDataFromServer();
    }).catch(err => { showLoading(false); showToast('❌ Error al importar: ' + err.message, false); });
}
...
function cargarPlanillasDesdeServidor() {
    ...
    return apiCall('/api/planillas')
        .then(data => { planillas = data || []; guardarPlanillasLocal(); return planillas; })
        .catch(err => { console.log('Error al cargar planillas:', err); return []; });
}
...
function iniciarPollingPlanillas() {
    detenerPollingPlanillas();
    if (!token) return;
    planillasInterval = setInterval(() => { if (!document.hidden) sincronizarPlanillas(); }, 5000);
}
...
function sincronizarPlanillas() {
    if (!token) return;
    apiCall('/api/planillas').then(data => {
        ...
        if (JSON.stringify(planillas) !== JSON.stringify(nuevasPlanillas)) {
            ...
            if (currentTab === 'planillasRecibidas' && currentUser?.rol === 'admin') renderPlanillasRecibidasUI(document.getElementById('planillasRecibidasList'));
            if (currentTab === 'planilla') actualizarContadorPlanillas();
        }
    }).catch(() => {});
}
...
function actualizarContadorPlanillas() {
    ...
    if (badgeEl) badgeEl.textContent = planillas.length > 0 ? planillas.length : '';
}
...
function registrarPlanilla() {
    ...
    if (currentUser && USUARIOS_LOCALES[currentUser.username]) {
        const nuevaPlanilla = {
            id: Date.now(), fecha, tipo, clasificacion, modulo, descripcion, horas,
            repuesto: repuesto || '', observaciones: observaciones || '',
            usuario: currentUser.username, tecnico: tecnico, timestamp: new Date().toISOString()
        };
        planillas.unshift(nuevaPlanilla);
        localStorage.setItem('panol_planillas', JSON.stringify(planillas));
        showLoading(false);
    ...
    apiCall('/api/planillas', {
        method: 'POST',
        body: JSON.stringify({ fecha, tipo, clasificacion, modulo, descripcion, horas, repuesto, observaciones, tecnico: tecnico })
    }).then(data => {
        showLoading(false);
        if (data.success) {
...
function renderPlanillasRecibidas() {
    ...
    if (!container) return;
    if (currentUser?.rol === 'admin') {
        if (currentUser && USUARIOS_LOCALES[currentUser.username]) { renderPlanillasRecibidasUI(container); return; }
        showLoading(true);
        apiCall('/api/planillas').then(data => {
            planillas = data || [];
            guardarPlanillasLocal();
            ...
            if (!planillasInterval) iniciarPollingPlanillas();
        }).catch(err => { showLoading(false); showToast('Error al cargar planillas', false); renderPlanillasRecibidasUI(container); });
    } else { renderPlanillasRecibidasUI(container); }
}
...
function renderPlanillasRecibidasUI(container) {
    ...
    if (empleadosFiltrados.length === 0) {
        container.innerHTML = `<div class="planilla-vacia"><div class="icono">📭</div><div class="titulo">Sin planillas registradas</div><p>Los empleados aún no han registrado trabajos.</p></div>`;
        return;
    }
    let html = '';
    empleadosFiltrados.forEach(empleado => {
        ...
        planillasEmpleado.forEach(p => {
            ...
            if (planillasPorDia[dia] < 7) { planillasPorDia[dia]++; planillasMostrar.push(p); }
        });
            ...
            <div class="empleado-planilla-card">
                ...
                <div class="empleado-planilla-body" id="planillasBody_${empleado}">
                    ${planillasMostrar.length > 0 ? planillasMostrar.map(p => `
                        ...
                        </div>`).join('') : `<div class="planilla-vacia" style="padding:12px;"><p>Sin trabajos registrados</p></div>`}
                    ${totalPlanillas > 7 ? `<div style="text-align:center;font-size:12px;color:var(--sub);padding:8px;">... y ${totalPlanillas - 7} trabajos más</div>` : ''}
                </div>
            </div>`;
    });
    container.innerHTML = html;
...
function togglePlanillasEmpleado(empleado) {
    ...
    if (body) { body.classList.toggle('open'); if (icon) { icon.textContent = body.classList.contains('open') ? '✕' : '+'; icon.classList.toggle('open'); } }
}
...
function eliminarPlanilla(id) {
    ...
    showLoading(true);
    apiCall(`/api/planillas/${id}`, { method: 'DELETE' }).then(() => {
        showLoading(false);
        planillas = planillas.filter(p => p.id !== id);
        guardarPlanillasLocal();
        renderPlanillasRecibidasUI(document.getElementById('planillasRecibidasList'));
...
function exportarPlanillasExcel() {
    ...
    const data = datosExportar.map(p => ({
        'Fecha': p.fecha, 'Técnico': p.tecnico, 'Tipo': p.tipo, 'Clasificación': p.clasificacion || 'Orden de Trabajo',
        'Módulo': p.modulo, 'Descripción': p.descripcion, 'Horas': p.horas,
        'Repuesto': p.repuesto || 'Ninguno', 'Observaciones': p.observaciones || '', 'Registrado por': p.usuario
    }));
    const ws = XLSX.utils.json_to_sheet(data);
    ws['!cols'] = [{ wch: 12 }, { wch: 15 }, { wch: 12 }, { wch: 20 }, { wch: 40 }, { wch: 10 }, { wch: 20 }, { wch: 30 }, { wch: 15 }];
...
function initPlanillaFecha() {
    ...
    if (tecnicoInput && currentUser) tecnicoInput.value = currentUser.username;
}
...
function cargarOrdenesDesdePlanillas() {
    ...
    planillas.forEach(p => {
        ...
        if (!idsExistentes.has(id)) {
            ordenes.push({
                id: id, fecha: p.fecha || '', maquina: p.modulo || '', falla: p.descripcion || '',
                clasificacion: p.clasificacion || 'Orden de Trabajo', tecnico: p.tecnico || p.usuario || '',
                horas: p.horas || 0, repuestos: p.repuesto || '', solucion: '', operativa: 'SI',
                tipoOrden: p.tipo || '', comentarios: p.observaciones || '', _origen: 'planilla'
            });
            idsExistentes.add(id);
            contador++;
        }
    });
    if (contador > 0) guardarOTLocal();
}

function initOrdenes() {
    const saved = localStorage.getItem('ot_ordenes_v2');
    if (saved) { try { const parsed = JSON.parse(saved); if (parsed.length > 0) ordenes = parsed; } catch(e) {} }
    if (currentUser?.rol === 'admin') cargarOrdenesDesdePlanillas();
    renderTablaOT();
}
...
function renderTablaOT() {
    ...
    let filtrados = ordenes.filter(o => {
        const matchSearch = !search || (o.id || '').toString().toLowerCase().includes(search) || (o.maquina || '').toLowerCase().includes(search) || (o.tecnico || '').toLowerCase().includes(search) || (o.falla || '').toLowerCase().includes(search);
    ...
    document.getElementById('promHorasOT').textContent = filtrados.length ? (sumHoras / filtrados.length).toFixed(1) : '0.0';
    document.getElementById('totalRepuestosOT').textContent = filtrados.filter(o => o.repuestos && o.repuestos.trim()).length;
    ...
    if (filtrados.length === 0) { tbody.innerHTML = ''; if (empty) empty.style.display = 'block'; return; }
    if (empty) empty.style.display = 'none';
    tbody.innerHTML = filtrados.map((o) => {
        const formatearFecha = (fecha) => {
            if (!fecha) return '';
            if (fecha instanceof Date) return `${String(fecha.getDate()).padStart(2, '0')}/${String(fecha.getMonth() + 1).padStart(2, '0')}/${String(fecha.getFullYear()).slice(-2)}`;
            const fechaStr = String(fecha);
            if (fechaStr.includes('-')) { const partes = fechaStr.split('-'); if (partes.length === 3) return `${partes[1]}/${partes[2]}/${partes[0].slice(-2)}`; }
            return fechaStr;
        };
        ...
        return `<tr>
            ...
            <td>${o.repuestos || '—'}</td>
            <td style="text-align:center;"><button class="btn-accion" onclick="verDetalleOT('${o.id}')">👁️</button></td>
        </tr>`;
    }).join('');
}
...
function cargarSelectoresOT() {
    ...
    if (selMaq) selMaq.innerHTML = '<option value="">Seleccionar máquina</option>' + maquinasList.map(m => `<option value="${m}">${m}</option>`).join('');
    if (selTec) selTec.innerHTML = '<option value="">Seleccionar técnico</option>' + tecnicosList.map(t => `<option value="${t}">${t}</option>`).join('');
}

function abrirFormularioOT() {
    if (currentUser?.rol !== 'admin') { showToast('Solo administradores pueden crear órdenes', false); return; }
...
function guardarOT(e) {
    ...
    if (!fecha || !maquina || !tecnico) { showToast('Completá los campos obligatorios', false); return; }
    const nuevaOT = {
        id: id, fecha: fecha, maquina: maquina,
        falla: document.getElementById('otFalla').value.trim(),
        clasificacion: document.getElementById('otClasificacion').value,
        tecnico: tecnico,
        horas: parseFloat(document.getElementById('otHoras').value) || 0,
        repuestos: document.getElementById('otRepuestos').value.trim() || '',
        solucion: document.getElementById('otSolucion').value.trim() || '',
        operativa: document.getElementById('otOperativa').value,
        tipoOrden: document.getElementById('otTipoOrden').value || '',
        comentarios: document.getElementById('otComentarios').value.trim() || '',
        turno: document.getElementById('otTurno').value || '',
        modulo: document.getElementById('otModulo').value.trim() || '',
        _origen: 'admin'
    };
...
function exportarOTXLSX() {
    ...
    const datos = ordenes.map(o => ({
        'ID': o.id || '', 'Fecha': o.fecha || '', 'Máquina': o.maquina || '',
        'Falla': o.falla || '', 'Clasificación': o.clasificacion || '',
        'Técnico': o.tecnico || '', 'Horas': o.horas || 0,
        'Repuestos': o.repuestos || '', 'Solución': o.solucion || '',
        'Operativa': o.operativa || '', 'Comentarios': o.comentarios || ''
    }));
    const wb = XLSX.utils.book_new();
    ...
    reader.onload = function(e) {
        try {
            ...
            for (let sheetName of wb.SheetNames) {
                if (sheetName.toLowerCase().includes('datos') || sheetName.toLowerCase().includes('data')) { wsDatos = wb.Sheets[sheetName]; break; }
            }
            if (!wsDatos && wb.SheetNames.length > 0) wsDatos = wb.Sheets[wb.SheetNames[0]];
            if (!wsDatos) { showToast('No se encontró ninguna hoja con datos', false); return; }
            const data = XLSX.utils.sheet_to_json(wsDatos);
            if (!data || data.length === 0) { showToast('El archivo está vacío', false); return; }
            importDataOT = data.map(row => {
                ...
                if (fechaRaw !== null && fechaRaw !== undefined) fecha = String(fechaRaw);
                if (fecha && fecha.includes('/')) {
                    ...
                    try {
                        ...
                        fecha = `${excelDate.getFullYear()}-${String(excelDate.getMonth() + 1).padStart(2, '0')}-${String(excelDate.getDate()).padStart(2, '0')}`;
                    } catch(err) { fecha = ''; }
                }
                ...
                if (row['Tipo de Orden']) {
                    ...
                    if (tipoOrden.includes('Preventivo')) clasificacion = 'Preventivo';
                }
                let horas = 0;
                const horasRaw = row['Tiempo de trabajo'] !== undefined ? row['Tiempo de trabajo'] : (row['Horas'] !== undefined ? row['Horas'] : (row['horas'] !== undefined ? row['horas'] : 0));
                if (horasRaw !== null && horasRaw !== undefined && horasRaw !== '') horas = parseFloat(String(horasRaw)) || 0;
                let operativa = 'SI';
                const opStr = String(row['Operativa'] || row['operativa'] || '').toUpperCase();
                if (opStr === 'FALSE' || opStr === 'NO' || opStr === '0') operativa = 'NO';
                return {
                    id, fecha, maquina: maquina || row['Modulo Intervenido'] || row['Modulo'] || row['modulo'] || '',
                    falla: row['Falla'] || row['falla'] || row['FALLA'] || row['Descripción'] || row['descripcion'] || '',
                    clasificacion, tecnico, horas,
                    repuestos: row['Repuestos'] || row['repuestos'] || row['REPUESTOS'] || '',
                    solucion: row['Solucion'] || row['Solución'] || row['solucion'] || '',
                    operativa,
                    comentarios: row['Comentarios'] || row['comentarios'] || row['Observaciones'] || row['observaciones'] || ''
                };
            });
            importDataOT = importDataOT.filter(o => o.maquina || o.tecnico || o.falla);
            if (importDataOT.length === 0) { showToast('No se encontraron datos válidos', false); return; }
            ...
            preview.innerHTML = `
                ...
                <div class="preview-table-wrap">
                    <table>
                        ...
                        <tbody>${importDataOT.slice(0, 10).map(o => `<tr><td>${o.id}</td><td>${o.fecha}</td><td>${o.maquina || '—'}</td><td>${o.clasificacion}</td><td>${o.tecnico || '—'}</td><td>${o.horas}</td></tr>`).join('')}
                        ${importDataOT.length > 10 ? `<tr><td colspan="6" style="text-align:center;color:var(--sub);">... y ${importDataOT.length - 10} más</td></tr>` : ''}</tbody>
                    </table>
                </div>`;
            showToast('✅ Archivo procesado. ' + importDataOT.length + ' registros listos.');
        } catch (err) { showToast('❌ Error al leer el archivo: ' + err.message, false); }
    };
    reader.readAsArrayBuffer(file);
...
function confirmarImportacionOT() {
    if (!importDataOT || importDataOT.length === 0) { showToast('No hay datos para importar', false); return; }
    let contador = 0;
    importDataOT.forEach(o => {
        const existe = ordenes.find(ord => ord.id == o.id);
        if (!existe) {
            ordenes.push({ ...o, _origen: 'importado' });
            contador++;
        }
    });
    guardarOTLocal();
...
setTimeout(() => {
    if (document.getElementById('ordenesSection')) { initOrdenes(); cargarSelectoresOT(); }
}, 500);
